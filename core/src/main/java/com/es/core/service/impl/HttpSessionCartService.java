...
public class HttpSessionCartService implements CartService {
    ...
    public void addPhone(Long phoneId, Long quantity) {
        ...
        tryToAddItemToCart(phone, quantity, inCartQuantity);
    }
    ...
    public void update(Map<Long, Long> items) {
        for (Map.Entry<Long, Long> entry : items.entrySet()) {
            ...
            Phone phone = phoneDao.get(phoneId).orElseThrow(() -> new EntityNotFoundException("Phone", phoneId));
            tryToAddItemToCart(phone, quantity, 0L);
        }
    }
    ...
    private void tryToAddItemToCart(Phone phone, Long quantity, Long inCartQuantity) {
        if (quantity <= 0) {
